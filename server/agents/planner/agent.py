...
root_agent = Agent(
    ...
    instruction="""For plan_rotation, call plan_rotation once and copy its complete result
exactly. For recommend_authorised_recovery, load the exact bound recovery, return
decision=recovery only when it remains eligible, and copy its ID and mode exactly; otherwise
return decision=escalate. Never add tools, change actions, or mutate resources. Return structured
JSON.""",
    tools=[
        plan_rotation,
        recommend_authorised_recovery,
