...
function suggestedCapabilities(roles: ConnectionRole[], platform: string) {
  const values: Record<ConnectionRole, string[]> = {
    provider: ["provider.listCredentialMetadata", "provider.createCredential", "provider.getCredentialStatus", "provider.revokeCredential", "provider.testCredential"],
...
export function ConnectionSetup({ onClose, playbooks, onChanged, onCreated, initialRoles = ["provider"], initialPlatform = "" }: { onClose: () => void; playbooks: Playbook[]; onChanged: () => Promise<void>; onCreated?: (connection: Connection) => Promise<void>; initialRoles?: ConnectionRole[]; initialPlatform?: string }) {
  ...
  const [testPath, setTestPath] = useState("/credential")
  const [testProviderIdField, setTestProviderIdField] = useState("id")
  const [credentialScheme, setCredentialScheme] = useState<"bearer" | "header" | "basic">("bearer")
  ...
  function canContinue() {
    ...
    if (step === 1) return Boolean(resources.trim()) && (connectionInterface === "browser" ? Boolean(playbookVersion && sessionContainer.trim()) : Boolean(authorizationReference.trim() && (!connectionRoles.includes("provider") || (baseUrl.trim() && requestHeader.trim() && testPath.trim() && testProviderIdField.trim() && credentialHeader.trim()))))
    return true
  }
  ...
  async function submit() {
    ...
    const connection: Connection = {
      ...
      http: isProviderApi ? { base_url: baseUrl.trim(), auth: { scheme: requestScheme, header: requestHeader.trim(), prefix: requestPrefix || null }, list_credentials: operation("GET", listPath), create_credential: operation("POST", createPath), revoke_credential: operation("DELETE", revokePath), test_credential: { ...operation("GET", testPath), list_items: null, provider_id_field: testProviderIdField.trim() }, credential_auth: { scheme: credentialScheme, header: credentialHeader.trim(), prefix: credentialPrefix || null } } : null,
      playbook_id: null,
      playbook_version_id: null,
      status: connectionRoles.includes("provider") || connectionInterface === "browser" ? "setup-required" : "ready",
      authenticated_at: connectionRoles.includes("provider") || connectionInterface === "browser" ? null : timestamp,
      authorization_expires_at: null,
      last_validated_at: connectionRoles.includes("provider") || connectionInterface === "browser" ? null : timestamp,
