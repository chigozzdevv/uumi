...
export function ConnectionsPage() {
  ...
  if (currentSelected) return <div className="page">
    <PageHeader eyebrow="Connections" title={currentSelected.display_name} description="Authorised access FireKey can use for typed operations." onBack={() => setSelected(null)} actions={currentSelected.interface === "browser" && currentSelected.playbook_version_id ? <Button onClick={() => openBrowser(currentSelected)} disabled={!sessionContainer.trim() || open.isPending}>Open browser <ExternalLink className="size-3.5" /></Button> : undefined} />
  ...
  return <div className="page">
    <PageHeader title="Connections" description="Authorise provider, runtime, secret-store, telemetry, and incident systems." actions={<Button onClick={() => setCreating(true)}><Plus className="size-3.5" /> Add connection</Button>} />
