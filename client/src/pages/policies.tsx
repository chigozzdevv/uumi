...
export function PoliciesPage() {
  ...
  if (selected) return <div className="page">
    <PageHeader eyebrow="Policies" title={selected.name} description="The safety gates that control when automation may proceed and which operations need approval." onBack={() => setSelected(null)} />
  ...
  return <div className="page">
    <PageHeader title="Policies" description="Define triggers, approval boundaries, evidence gates, and recovery limits for rotations." actions={<Button onClick={() => setCreating(true)}><Plus className="size-3.5" /> Add policy</Button>} />
...
function PolicySetup({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <SetupPage eyebrow="Policies" title="Add policy" description="Configure the conditions and evidence gates that govern credential rotation." steps={steps} current={step} onBack={() => setStep((value) => value - 1)} onCancel={onClose} error={mutation.error?.message} primary={step < 3 ? <Button onClick={() => setStep((value) => value + 1)} disabled={!canContinue}>{primary}</Button> : <Button onClick={submit} disabled={mutation.isPending}>{mutation.isPending ? "Saving…" : primary}</Button>}>
