  ...
  return (
    <div className="page">
      ...
      <Modal isOpen={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.name ?? "Policy"}>
        {selected && <><Section title="Policy"><DetailList><Detail label="Version">{selected.latest_version}</Detail><Detail label="Status"><Badge variant={selected.active_version_id ? "healthy" : "warning"}>{selected.active_version_id ? "Active" : "Draft"}</Badge></Detail><Detail label="Updated">{formatDate(selected.updated_at, true)}</Detail></DetailList></Section><Section title="Automation"><DetailList><Detail label="Triggers">{selected.automatic_triggers?.join(", ") ?? "Not configured"}</Detail><Detail label="Protected operations">{selected.protected_operations?.join(", ") ?? "None"}</Detail><Detail label="Rollout">{selected.rollout?.map((value) => `${value}%`).join(" → ") ?? "Not configured"}</Detail></DetailList></Section></>}
      </Modal>
    </div>
  )
}
