...
export function IncidentsPage({ onNavigateRotation }: { onNavigateRotation: (runId: string) => void }) {
  ...
  return (
    <div className="page">
      <PageHeader section="Operations · Incidents" />
      ...
      <Modal
        ...
        title={selected ? titleCase(selected.source) : "Incident"}
        actions={selected?.run_id && <Button onClick={() => onNavigateRotation(selected.run_id!)}>Open rotation <ArrowUpRight className="size-3.5" /></Button>}
      >
        {selected && <>
          ...
          <Section title="Source"><DetailList><Detail label="Repository">{selected.resource.repository ?? "—"}</Detail><Detail label="Project">{selected.resource.project ?? "—"}</Detail><Detail label="Service">{selected.resource.service ?? "—"}</Detail><Detail label="Observed">{formatDate(selected.created_at, true)}</Detail></DetailList></Section>
          <Section title="Credential"><div className="rounded-2xl border border-[var(--border)] bg-white/65 p-5"><div className="flex items-center gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-xl bg-[var(--red-soft)] text-[var(--red)]"><ShieldAlert className="size-4" /></span><div className="text-[12px] font-semibold">{credentialName(selected.credential_id)}</div></div></div></Section>
        </>}
      </Modal>
    </div>
  )
