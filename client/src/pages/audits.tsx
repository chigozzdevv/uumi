...
export function AuditsPage() {
  ...
  if (selected) return <div className="page"><PageHeader eyebrow="Audit" title={titleCase(selected.kind)} onBack={() => setSelected(null)} /><Section title="Event"><DetailList><Detail label="Sequence">#{selected.sequence}</Detail><Detail label="Actor">{actorName(selected.actor_id)}</Detail><Detail label="Resource">{resourceName(selected.resource)}</Detail><Detail label="Occurred">{formatDate(selected.occurred_at, true)}</Detail><Detail label="Region">{selected.region}</Detail><Detail label="Status"><Badge variant="healthy">Verified</Badge></Detail></DetailList></Section></div>
