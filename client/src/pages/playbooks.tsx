...
export function PlaybooksPage() {
  ...
  if (selected) return <div className="page">
    <PageHeader eyebrow="Playbooks" title={selected.name} onBack={() => setSelected(null)} />
  ...
  return <div className="page">
    <PageHeader title="Playbooks" actions={<Button onClick={() => setCreating(true)}><Plus className="size-3.5" /> Add playbook</Button>} />
...
function PlaybookSetup({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <SetupPage eyebrow="Playbooks" title="Add playbook" steps={steps} current={step} onBack={() => setStep((value) => value - 1)} onCancel={onClose} error={mutation.error?.message} primary={step < 2 ? <Button onClick={() => setStep((value) => value + 1)} disabled={!canContinue()}>{primary}</Button> : <Button onClick={submit} disabled={mutation.isPending}>{mutation.isPending ? "Building…" : primary}</Button>}>
