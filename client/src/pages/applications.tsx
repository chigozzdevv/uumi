...
export function ApplicationsPage() {
  ...
  if (selected) return <div className="page">
    <PageHeader eyebrow="Applications" title={selected.display_name} onBack={() => setSelected(null)} actions={<Button onClick={() => setAddingService(true)}><Plus className="size-3.5" /> Add service</Button>} />
  ...
  return <div className="page">
    <PageHeader title="Applications" actions={<Button onClick={() => setCreating(true)}><Plus className="size-3.5" /> Add application</Button>} />
...
function ApplicationSetup({ onClose, connections, onCreated }: { onClose: () => void; connections: Connection[]; onCreated: () => Promise<void> }) {
  ...
  return <SetupPage eyebrow="Applications" title="Add application" steps={setupSteps} current={step} onBack={() => setStep((value) => value - 1)} onCancel={onClose} error={mutation.error?.message} primary={step < 2 ? <Button onClick={() => setStep((value) => value + 1)} disabled={!canContinue}>{step === 0 ? "Continue to runtime" : "Review application"}</Button> : <Button onClick={submit} disabled={mutation.isPending}>{mutation.isPending ? "Adding…" : "Add application"}</Button>}>
