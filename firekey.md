...
└── notification-worker               Application
    ├── Staging                        Environment
    │   └── notification-worker        Consumer service deployment
...
A consumer service is a deployed runtime resource that reads and uses a credential. Its application name is derived from that resource instead of being entered as a separate business grouping. The runtime is the infrastructure that executes the service and controls its deployment, configuration, traffic, health, logs, and rollback.
...
Only runtime services explicitly bound to a credential are consumers of that credential.
...
Credential
  ...
  Runtime connection and discovered service
  Discovered runtime secret binding
...
Secret store
  ...
  Resource: projects/production/secrets/password-emailer
...
The credential's Consumers tab shows every bound runtime deployment:

```text
Credential → every service that consumes it
```

Application, environment, and consumer-service records are derived from the selected runtime service
and committed atomically with the credential. They remain execution and audit metadata rather than a
separate dashboard inventory journey.
...
Each stage opens into its own execution record: status and timing, completed checks, evidence count,
the agent's typed decision and concise rationale, and any Computer Use objectives and outcomes. The
dashboard never exposes encrypted thought signatures, unrestricted tool output, secret references,
...
→ enter approved inputs
→ create the replacement
→ capture its one-time value through Secure Capture
→ locate the previous credential by provider ID
...
credential. Every browser-managed credential using that connection inherits its pinned version.
API provider connections do not use Playbooks; their deterministic connector operations are
declared by the connection and implemented by the connector.

```text
Browser Playbook version
        ↓
Browser provider connection
        ↓
Managed credentials for that platform
```

The Playbooks page lists reusable browser procedures:

```text
Playbooks                                                        [+]

Name                              Platform          Connections   Status
Internal Vendor Key Rotation      Internal Vendor   2             Published
Partner Portal Key Rotation       Partner Portal    0             Draft
```

The `+` action opens a two-step Playbook Builder:
...
Text is sanitised before model analysis. Uploaded videos are checksum-bound in the protected
walkthrough bucket; Cloud Storage references are copied from an exact source generation into that
bucket before analysis. Google Cloud Video Intelligence extracts sanitised speech, screen text, and shot boundaries, then the Gemini
Playbook Builder Agent converts that evidence into a structured draft. The second step shows the
...
```text
Overview | Source | Actions | Checkpoints | Versions | Connections
```
...
login_url_pattern: https://login.vendor.example.com/*

actions:
  - operation: navigate
    objective: Open API key administration
    checkpoint: provider key list
  - operation: create
    objective: Create the replacement with approved name and scopes
    secure_capture: generated_credential
  - operation: revoke
    objective: Locate the prior provider ID and revoke it
```
...
```mermaid
flowchart LR
    A["Video or text source"] --> B["Sanitisation and Video Intelligence"]
    B --> C["Gemini Playbook Builder Agent"]
    C --> D["Structured draft"]
    D --> E["Structural and security validation"]
    E --> F["Author review"]
    F --> G["Publish immutable version"]
    G --> H["Attach to browser connection"]
    H --> I["Open browser and authenticate"]
```
...
OAuth, workload identity, API-key authentication, and browser sessions are steps inside the
appropriate integration flow; they are not separate catalogue cards. Provider-specific setup stays
inside connectors while the core persists the same typed connection contract.
...
A browser provider connection additionally records one published Playbook version. Its setup flow is:

```text
Choose provider role and browser interface
→ attach a compatible published Playbook
→ create the connection as Setup required
→ Open browser
→ user completes provider login and MFA inside the isolated browser
→ Auth Broker stores the encrypted session outside agent context
→ connection becomes Ready
```

If the browser session expires, the connection becomes `Reauthentication required`. A run that
needs it pauses at the current checkpoint; the user opens the isolated browser again, renews the
session, and the coordinator resumes. User involvement establishes authentication only. The agent
remains the coordinator and deterministic components remain the executors.
...
Firestore stores only the searchable audit index. Canonical audit events are append-only, hash-chained, and written to a dedicated regional Cloud Logging bucket with retention locked and CMEK where required. Larger evidence objects, including the exact sanitised browser images sent to Gemini and exported reports, are kept in a retention-locked Cloud Storage evidence bucket and referenced by content hash. Neither store contains plaintext credential values or raw secret-bearing browser frames.
...
the administrator selects another ready repository.
...
The provider connection fixes the execution interface:

1. An API connection exposes typed provider operations and needs no Playbook.
2. A browser connection pins a published Playbook and uses Gemini Computer Use in a one-run isolated Compute Engine browser VM.
...
flowchart TD
    ...
    F --> G["Verify provider, secret, and runtime state"]
...
- Provider credential testing and runtime binding inspection are available.
...
4. **Coverage gate** — every required consumer is included in the plan.
...
When approval is required, the approver may:
...
3. Uses the typed provider connector, or the attached browser Playbook when the provider connection is browser-based, to disable, revoke, or delete the provider-side old credential.
...
Provider, secret-store, and runtime verification pass
...
- Uses typed API operations for API provider connections or binds the published Playbook from a browser provider connection.
...
Registered skills: `build_playbook`, `analyse_walkthrough`, and `validate_playbook`.
...
- Identifies goals, inputs, page states, browser actions, outputs, and unsafe or ambiguous controls.
...
- Validates expected checkpoints, selector determinism, domains, and Secure Capture boundaries.
- Revises a draft from authorised reviewer feedback and optional sandbox evidence.

It cannot publish its own output. Deterministic structural validation and an authorised reviewer
are required before a version becomes published.
...
- Runs typed provider, secret-store, and runtime probes from versioned definitions.
- Checks provider, secret-store, deployment, optional telemetry, coverage, and rollback gates.
- Correlates generation IDs, logs, metrics, and traces using deterministic rules when telemetry is connected.
...
The Playbook service stores structured, versioned browser procedures and the evidence used to
build them. Uploaded videos and walkthrough recordings are retained according to organisation
policy in protected object storage; the executable Playbook is stored separately from its
teaching material.

The Playbook Builder Agent can interpret demonstrations, text, and links, but it produces only a
draft. The service applies schema validation, browser-tool and domain allowlists, deterministic
checkpoint checks, Secure Capture invariants, authorised publication, and immutable versioning
before a Playbook can attach to a browser connection.
...
Every published Playbook declares:

- Platform and allowed browser domains.
- Login URL checkpoint.
- Ordered create and revoke actions.
- Deterministic selectors, expected page states, outputs, and bounded adaptation rules.
- The Secure Capture field and provider-ID selector for a generated one-time credential.
- Evidence checks, timeouts, and retry limits.
...
| Provider, secret-store, or runtime verification fails | Capture evidence, preserve the old key, diagnose, retry, or abandon the candidate |
...
| Consumer binding | Joins a managed credential to a service, environment, runtime binding, currently deployed generation, target generation, and its latest verification report |
...
- An authorised operator can watch or take over a Computer Use run while only exact sanitised model inputs and visible returned events are retained.
